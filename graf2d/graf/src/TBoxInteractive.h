...
class TBoxInteractive : public TVirtualPad::TInteractive {
   public:
      ...
      void CalcPixelCoord(TVirtualPad &parent, Double_t x1, Double_t y1, Double_t x2, Double_t y2);

      Bool_t SelectCorner(Int_t px, Int_t py, Bool_t canX = kTRUE, Bool_t canY = kTRUE);

      Bool_t IsResizing() const;

      Bool_t IsOpaque(TVirtualPad &parent) const;

      Bool_t ProcessMouseMove(TVirtualPad &parent, Int_t px, Int_t py, Bool_t canX = kTRUE, Bool_t canY = kTRUE, Double_t aspectRatio = 0.);
